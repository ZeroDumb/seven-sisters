...
FAIL_COUNTER_PATH = os.path.join(os.path.dirname(__file__), "alice_fail.count")

# Required tools for Alice
TOOL_NAME = "tools/alice_recon.sh"
# Alice's tool is safe to use in safe mode
SAFE_MODE_ALLOWED = True
# Alice's minimum required level
REQUIRED_LEVEL = 0
...
def speak(msg, style="normal"):
    """Enhanced speak function with different styles for Alice's character"""
    if style == "command":
...
def handle_seven_command(command_type, args):
    ...
    if command_type == "recon":
        speak("The Queen has spoken...", "command")
        time.sleep(0.3)
        speak("Down the rabbit hole we go!", "command")
        time.sleep(0.3)
        target = args.get('target', 'unknown')
        speak(f"Curiouser and curiouser... a target to investigate: {target}", "thought")
        run_tool(target)
        return {'success': True, 'target': target}
    elif command_type == "status":
        speak("The Queen requests my status...", "command")
        time.sleep(0.3)
        speak("All systems operational, tea party protocols engaged!", "thought")
        return {'status': 'ready'}
    else:
        speak("What a strange command from the Queen...", "thought")
        return {'status': 'unknown_command'}

def setup_commands(comm_manager):
    """Set up command handlers for Alice."""
    def handle_status_query(args):
        return {'status': 'operational'}
    
    def handle_recon_command(args):
        ...
        for agent in config.get("agents", []):
            if agent.get("name") == SISTER_NAME:
                ...
                if new_level > current_level:
                    speak(f"🔼 My curiosity has increased to level {new_level}!", "command")
                elif new_level < current_level:
                    speak(f"🔽 My curiosity has decreased to level {new_level}...", "command")
                else:
                    speak(f"⚖️ My curiosity remains at level {new_level}", "command")
                break
    ...
    def handle_activate_command(args):
        ...
        speak("🎩 The Queen has summoned me! Time for an adventure...")
    ...
    comm_manager.command_handler.register_command('recon', handle_recon_command)
...
def run_tool(target):
    ...
    if tool_exists:
        speak(f"🐇 Starting reconnaissance on target: {target}", "command")
        write_output(SISTER_NAME, target, "🐇 Beginning reconnaissance operations")
        speak("Following the white rabbit...", "thought")
        subprocess.run(["bash", TOOL_NAME, target])
        write_output(SISTER_NAME, target, "✅ Reconnaissance complete")
        speak("The rabbit has led us to interesting places...", "thought")
    else:
        speak("🐇 My reconnaissance tools are missing...", "command")
        write_output(SISTER_NAME, target, "❌ Tool script not found: alice_recon.sh")
        speak("The rabbit hole seems to be blocked...", "thought")
...
def boot_sequence():
    speak("Initializing reconnaissance protocols...", "command")
    time.sleep(0.3)
    speak("Tea party protocols online.", "thought")
    time.sleep(0.3)
    speak("Curiosity levels optimal.", "thought")
    time.sleep(0.3)
    speak("We're all mad here...", "command")
    time.sleep(0.3)
    speak("Ready to serve the Queen.", "command")
...
def main():
    ...
    try:
        ...
        if fails >= 3:
            speak("Curiouser and curiouser! I've fallen down the wrong rabbit hole...")
            write_output(SISTER_NAME, "unknown", "❌ Critical error: Config loading failed 3+ times")
        else:
            speak("Oh dear, I seem to have lost my way in Wonderland...")
            write_output(SISTER_NAME, "unknown", "⚠️ Config loading failed")
        sys.exit(1)
    if not config:
        speak("I don't even know who I am in this Wonderland... identity crisis!")
    ...
    if not config.get("enabled", False):
        speak("I'm not in the mood for adventures today... maybe later.")
    ...
    if safe_mode and not SAFE_MODE_ALLOWED:
        speak("🛑 Safe mode is on. No adventures today.")
    ...
    if current_level < REQUIRED_LEVEL:
        speak(f"⚙️ Too curious for my current level ({current_level}). I need level {REQUIRED_LEVEL} or higher.")
    ...
    if not verify_tools([TOOL_NAME], SISTER_NAME, speak):
        speak("🎩 My Wonderland powers are incomplete... tools missing.")
    ...
    while True:
        try:
            ...
            if comm_manager.termination_signal_received:
                speak("Time to return to Wonderland...")
                break
            ...
            speak(f"Curiouser and curiouser! Something's wrong: {str(e)}")
